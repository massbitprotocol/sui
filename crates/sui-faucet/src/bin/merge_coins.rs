...
async fn _split_coins_equally(
    ...
    let signature = wallet
        ...
        .unwrap();
    let tx = Transaction::from_data(tx_data, Intent::sui_transaction(), vec![signature]).unwrap();
    let resp = client
        .quorum_driver_api()
...
async fn _merge_coins(gas_coin: &str, mut wallet: WalletContext) -> Result<(), anyhow::Error> {
    ...
    for chunk in small_coins.chunks(254) {
        ...
        let signature = wallet
            ...
            .unwrap();
        let tx =
            Transaction::from_data(tx_data, Intent::sui_transaction(), vec![signature]).unwrap();
        client
            .quorum_driver_api()
            .execute_transaction_block(
                tx.clone(),
