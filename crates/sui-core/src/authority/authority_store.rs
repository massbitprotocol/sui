...
impl AuthorityStore {
    ...
    pub fn check_receiving_objects(
        ...
        for (object_id, version, _) in receiving_objects {
            ...
            // TODO(tzakian): check the marker table here as well -- if the object ID exists and it
            // doesn't exist in the marker table, then we know this is the most recent version of the
            // object without doing a range query. The other option is to perform a range query here
            // for each receiving object, and then verify that the version we get back matches the
            // provided version.
        }
        Ok(())
    }
